<template>
  <div class="shadow-sm">
    ...
    <b-list-group class="shadow autocomplete-list" v-show="showList" @mousedown.prevent>
      ...
      <b-list-group-item v-for="poke in results" :key="poke.pokeID"
                         @click="handlePokemonSelected(poke)" button>
        <div class="">
          <div class="col-4 col-lg-6">{{ poke.pokeName }}</div>
...
@Component
export default class PokemonSearch extends Mixins(GlobalMixin) {
  ...
  @Prop(Array) readonly pokemon!: Pokemon[]; // New prop for the array to be searched
  ...
  results: Pokemon[] = []  // array of student object returned by the api
  ...
  filterPokemon() {
    ...
    this.results = this.pokemon.filter(item =>
      (item.pokeName ?? '').toLowerCase().includes(this.query.toLowerCase())
    );
  ...
  handlePokemonSelected(pokemon: Pokemon): Pokemon {
    this.hasFocus = false;
    this.selectedPokemon = pokemon;
    this.query = pokemon.pokeName!;
    return pokemon;
  }
